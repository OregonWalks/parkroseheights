...
var cartoDBkey = "dab48d14a82a8eacef5e64fca9fdd15c54590247";
var buttonsHeight;
var i_mapArea;
...
function initAreaMap() {
    ...
    var intersections = new OpenLayers.Layer.Vector("intersections", {
        ...
        protocol: new OpenLayers.Protocol.Script({
			url: "http://travisdriessen.cartodb.com/api/v2/sql",
    ...
    map = new OpenLayers.Map({
        ...
        center: new OpenLayers.LonLat(-13642100, 5707300), 
        zoom:15
    });
};
...
function initDetailMap() {
	...
	ramps = new OpenLayers.Layer.Vector("ramps", {
		...
		protocol: new OpenLayers.Protocol.Script({
			url: "http://travisdriessen.cartodb.com/api/v2/sql",
			params: {q: "select * from ramps where nodeid = "+intersectionID+ " order by sequence asc, down_ramp asc", format: "geojson"},
			format: geoJSONparser,
			callbackKey: "callback"
		}),
			...
			"featuresadded": function() {
				...
				for (var i=0; i<this.features.length; i++) {
					...
					for (var attr in attributes){
						if (attr == 'sequence') {attrs.sequence = attributes[attr];}
...
var updateNotes = function(){
	...
	var noteText;
	jQuery.getJSON("http://travisdriessen.cartodb.com/api/v2/sql", query, function(notes){
...
var updateRampNotes = function(){
	...
	for (var j=1; j<rampNoteChoiceCount; j++){rampNotes[j] = false;};
