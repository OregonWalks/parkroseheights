...
jQuery(document).ready(function() {
    ...
    jQuery("#locate").click(function(){
        ...
        jQuery("#intersectionpage").on("pageshow", function(event, ui) {
                ...
                if (null == i_map) {
                        ...
                        ramps.protocol.params.q = "select * from ramps where nodeid = "+intersectionID+ " order by sequence asc, down_ramp asc";
        ...
        jQuery("#sortOf").click(function() {
                ...
                rampID = (intersectionID*10000)+(10*attr.sequence)+((attr.down_ramp == "cw")?0:1);
                */
                moveCW();
                /*
                fromRampNotes = true;
        ...
        jQuery("#save").click(function(){
                var attr;
                var allNone = "n";
                var pdxTime = new Date();
                var query;
                for (f in rampAttrs.features) {
                        attr = rampAttrs.features[f].attributes;
                        query = "q=UPDATE ramps SET state = '"+attr.state+"' WHERE nodeid = "+intersectionID+" AND sequence = "+attr.sequence+" AND down_ramp = '"+attr.down_ramp+"' &api_key="+cartoDBkey;
                        jQuery.post("http://travisdriessen.cartodb.com/api/v2/sql", query);
                        if (attr.state != "none") {allNone = "y"};
                };
                query = "q=UPDATE intersections SET evaluated = '"+allNone+"', hour_min = '"+pdxTime+"' WHERE node_id = "+intersectionID+"&api_key="+cartoDBkey;
            jQuery.post("http://travisdriessen.cartodb.com/api/v2/sql", query, function(data) {areaMapStrategy.refresh({force:true});});
  ...
  /*  jQuery('#popup').live('pageshow',function(event, ui){
        ...
        //put the query together to get info from the corners table in CartoDB
                ...
                var url = "http://travisdriessen.cartodb.com/api/v2/sql?"+stNmQuery;
